...
EOT,
  ...
  /**
   * Creates tasks to copy assets to include in the archive.
   *
   * @return list<\Robo\Contract\TaskInterface>
   *   List of tasks.
   */
  protected function createCopyTasks(): array {
    $config_copy_dirs = $this->getConfig()->get('release.copy');
    if (!$config_copy_dirs) {
      return [];
    }
    $copy_dirs = [];
    foreach ($this->getConfig()->get('release.copy') as $subdir) {
      $copy_dirs["_release_/$subdir"] = $subdir;
    }
    $task = $this->taskCopyDir($copy_dirs);
    return [$task];
  }
